...
This is a standalone project that can be used as-is to deploy the application on the hardware. However, it is also part of the [ST ModelZoo repo](https://github.com/STMicroelectronics/stm32ai-modelzoo-services) and is needed to deploy the image classification use case. The ModelZoo allows you to train, evaluate and deploy automatically any supported model. If you wish to use it as part of the ModelZoo, please refer to the [ModelZoo README](https://github.com/STMicroelectronics/stm32ai-modelzoo-services/blob/main/application_code/image_classification/STM32N6/README.md) on how to use it.
...
Three binaries must be programmed in the board external flash using the following procedure:
  ...
  4. Program `Binary/STM32N6_GettingStarted_ImageClassification.hex` (firmware application)
  5. Switch BOOT1 switch to Left position
  6. Power cycle the board

__Note__: The `Binary/STM32N6_GettingStarted_ImageClassification.hex` firmware is built for MB1939 STM32N6570-DK REV C02 with IMX335 Camera module.
...
STM32_Programmer_CLI -c port=SWD mode=HOTPLUG -el $DKEL -hardRst -w Binary/STM32N6_GettingStarted_ImageClassification.hex
```
